...
class Login {
    ...
    public function verify_login($post) {
        ...
        //update last_login field for current user
        $date = date('Y-m-d H:i:s');
        $updatelogin = $this->db->update('users', array('last_login' => $date ,), $user->ID);
    ...
    public function register($post) {
      ...
      // Required fields
        ...
        // compare first and second password entries
        if ($post['firstpassword']!=$post['password']){
          return array('status'=>0,'message'=>"Passwords don't match");
        }
      ...
      return array('status'=>0,'message'=>'wrong secret password');
    ...
    public function lost_password($post) {
        ...
        if ( false !== $sent ) {
            ...
            return array('status'=>1,'message'=>'Check your email for the password reset link (allow 15 minutes for delivery)');
        }
